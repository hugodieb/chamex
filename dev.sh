...
workon chamex  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development chamex npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop chamex
    docker rm chamex
    docker run --name chamex -d --env-file /home/ubuntu/chamex.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/chamex:/dkdata \
        chamex start_web.sh
}
...
echo_green "Welcome to chamex's dev env"
