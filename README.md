...
```bash
dk bash  # starts bash inside "chamex" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and chamex.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `chamex` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The chamex container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv chamex -p python3  # creates a python3 virtualenv
